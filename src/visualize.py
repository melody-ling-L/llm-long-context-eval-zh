...
def plot_niah_heatmap(
    ...
    if save:
        path = FIGURES_DIR / f"niah_heatmap_{model}.png"
...
def plot_niah_heatmap_interactive(
    ...
    if save_html:
        path = FIGURES_DIR / f"niah_heatmap_{model}_interactive.html"
...
def plot_accuracy_by_length(
    ...
    if save:
        path = FIGURES_DIR / "accuracy_by_length.png"
        fig.savefig(path, dpi=150, bbox_inches="tight")
        print(f"✅ 已保存: {path}")
...
def plot_position_bias(
    ...
    if save:
        path = FIGURES_DIR / "position_bias.png"
        fig.savefig(path, dpi=150, bbox_inches="tight")
        print(f"✅ 已保存: {path}")
